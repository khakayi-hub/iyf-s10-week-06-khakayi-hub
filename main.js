    ...
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            if (userId > 0) {
                ...
                reject("Cannot fetch posts: Invalid user ID");
            }
        }, 1000);
    });
}
    ...
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            if (postId > 0) {
                ...
                reject("Cannot fetch comments: Invalid post ID");
            }
        }, 1000);
    });
}
...
Promise.race([fast, slow])
    ...
    //Build to fetch data for users simultaneously and display them all at once
    //Reusing my existing function
function getUserData(userId) {
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            if (userId > 0) {
...
function getDataWithPromises() {
    return getUserData(1)
        ...
        .then(posts => getPostComments(posts[0].postId))
...
async function getAllUsers() {
    ...
    document.body.innerHTML += `<h2>All Users</h2>`;
...
async function getUserPosts() {
    ...
    document.body.innerHTML += `<h2>User 1 Posts</h2>`;
...
//Add these features to the user directory
